...
def addVotes(votes: schema.VotesData, db:Session = Depends(get_db), current_user = Depends(oAuth.getCurrentUser)):
    ...
    if(votes.action == 1):
        if found_vote_data:
            raise HTTPException(status_code = status.HTTP_409_CONFLICT, detail = "you have already voted. ypu cannot vote again!")
        ...
        if not found_vote_data:
            raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "Cannot find vote!")
